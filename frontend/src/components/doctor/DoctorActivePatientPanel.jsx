...
export default function DoctorActivePatientPanel({ patient, onClose, testName, onTestNameChange, prescriptionText, onPrescriptionTextChange, followupDays, onFollowupDaysChange, onPrescribeTest, onIssuePrescription, onNotesSave }) {
  return (
    <div className="card glass-panel animate-fade-in" style={{ padding: 'var(--spacing-xl)' }}>
      ...
      {patient.lab_tests?.length > 0 && (
        <div style={{ marginBottom: 'var(--spacing-xl)' }}>
          ...
          {patient.lab_tests.map((test, idx) => (
      ...
      <div style={{ background: 'var(--bg-main)', padding: '2rem', borderRadius: 'var(--radius-md)' }}>
        ...
        <form onSubmit={onPrescribeTest} style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
          <input type="text" required placeholder="e.g. Complete Blood Count (CBC)" value={testName} onChange={e => onTestNameChange(e.target.value)} style={{ flex: 1, padding: '0.75rem', borderRadius: '4px', border: '1px solid #ccc' }} />
        ...
        <form onSubmit={onIssuePrescription} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <textarea required placeholder="Rx: Amoxicillin 500mg, take 1 cap twice daily for 7 days" value={prescriptionText} onChange={e => onPrescriptionTextChange(e.target.value)} style={{ width: '100%', padding: '0.75rem', borderRadius: '4px', border: '1px solid #ccc', minHeight: '80px' }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem', background: 'white', borderRadius: '4px', border: '1px solid #e2e8f0' }}>
            ...
            <select value={followupDays} onChange={e => onFollowupDaysChange(Number(e.target.value))} style={{ padding: '0.4rem', borderRadius: '4px', border: '1px solid #ccc' }}>
