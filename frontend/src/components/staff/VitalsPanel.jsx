...
export default function VitalsPanel({ queue, vitalsForm, vitalsData, onVitalsFormChange, onVitalsDataChange, onSubmitVitals, onCancelVitals }) {
  return (
    <>
        ...
        <div className="card glass-panel" style={{ borderLeft: '4px solid var(--primary)', marginBottom: 'var(--spacing-xl)' }}>
          ...
          <form onSubmit={onSubmitVitals} style={{ marginTop: '1rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div>
              ...
              <input required type="text" value={vitalsData.weight} onChange={e => onVitalsDataChange({ ...vitalsData, weight: e.target.value })} style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: '1px solid #ccc' }} placeholder="e.g. 70" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.2rem' }}>Heart Rate (bpm)</label>
              <input required type="text" value={vitalsData.hr} onChange={e => onVitalsDataChange({ ...vitalsData, hr: e.target.value })} style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: '1px solid #ccc' }} placeholder="e.g. 75" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.2rem' }}>Blood Pressure (mmHg)</label>
              <input required type="text" value={vitalsData.bp} onChange={e => onVitalsDataChange({ ...vitalsData, bp: e.target.value })} style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: '1px solid #ccc' }} placeholder="e.g. 120/80" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.2rem' }}>Temperature (°F)</label>
              <input required type="text" value={vitalsData.temp} onChange={e => onVitalsDataChange({ ...vitalsData, temp: e.target.value })} style={{ width: '100%', padding: '0.5rem', borderRadius: '4px', border: '1px solid #ccc' }} placeholder="e.g. 98.6" />
            </div>
