...
export function downloadPrescriptionPDF(rx, patientName) {
  const doc = new jsPDF();
...
export async function downloadDischargeSummary(apptId, patientName, apiFetch, notify) {
  try {
...
export function downloadInvoicePDF(inv, patientName) {
  const doc = new jsPDF();
